...
usage() {
  cat <<'EOF'
Uso:
  bash .engrama/engine/scripts/exec-bridge.sh --order <arquivo> --label <slug> [--sandbox <read-only|workspace-write>] [--date <YYYY-MM-DD>] [-- <flags extras do codex>]

Exemplo:
  bash .engrama/engine/scripts/exec-bridge.sh --order /tmp/ordem.md --label fatia-01 --sandbox workspace-write -- --model gpt-5.4
EOF
}
