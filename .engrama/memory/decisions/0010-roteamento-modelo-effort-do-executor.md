...
type: decision
status: active
touches: [memory/decisions/0003-executor-bridge-orquestrador-invoca-executor, memory/decisions/0005-orquestrador-qa-reexecucao-e-metricas, memory/decisions/0008-subagentes-so-na-lane-do-orquestrador]
date: 2026-06-20
...
Política de **roteamento de modelo + effort** do executor-bridge: o Orquestrador decide, por tarefa, qual modelo e esforço o `codex exec` usa — e qual a própria abordagem (solo/paralelo/profundidade de auditoria). Refina a "Escalonamento de força" da [[memory/decisions/0003-executor-bridge-orquestrador-invoca-executor]]. Postura: **conservador-pra-cima** (default alto; descer exige justificativa). **Modelos de EXECUÇÃO para escrever código; o papel de CRÍTICA usa o maior modelo aprovado** (`gpt-5.5`) — ver "Exceção" abaixo (decisão da Autoridade).

> **Template:** preencha os ids reais de modelo do seu projeto. Aqui usamos três faixas canônicas: `gpt-5.4-mini` (barato/rápido, execução), `gpt-5.4` (forte, execução) e `gpt-5.5` (o maior aprovado, reservado ao papel de crítica). A rubrica e a postura abaixo são portáveis; só a tabela tier→modelo precisa dos ids concretos.

## Knobs reais (executor)
- Modelo: flag de modelo do `codex exec` — execução usa `gpt-5.4-mini` (barato/rápido) e `gpt-5.4` (forte). `gpt-5.5` é **reservado ao papel de crítica** (ver Exceção). Modelos não aprovados não entram sem ordem.
...
> **Template:** confirme a sintaxe real das flags do seu `codex exec` (flag de modelo e flag de esforço) e o conjunto de níveis de effort que ele aceita.
...
**Default = T3 (`gpt-5.4` / high).** Descer para T1/T2 exige justificativa afirmativa; em dúvida, fica em T3 ou sobe.

| Tier | Quando (tem que se aplicar claramente) | Modelo | Effort |
|---|---|---|---|
| **T1** | trivial mecânico inequívoco: rename, format, port literal, mensagem, 1–2 linhas, regen de template determinístico | `gpt-5.4-mini` | low |
| **T2** | simples mas não-trivial, spec 100% clara, zero nuance de domínio, baixo risco | `gpt-5.4` | medium |
| **T3 (default)** | qualquer fatia padrão: lógica de negócio, contrato HTTP, e2e, refactor não-trivial; **dúvida cai aqui** | `gpt-5.4` | high |
| **T4** | governança/arquitetura/incidente · debugging com evidência conflitante · **sensível + agravante** (ver "risco define piso") | `gpt-5.4` *(salvo crítica → modelo máximo)* | **high** (default) |
| **T4+ (xhigh, só por gatilho)** | xhigh **só** quando: arquitetura nova, debugging com evidência contraditória, **ou 2 auditorias falhadas** no mesmo item | `gpt-5.4` | xhigh |

> **"Leve" estreitado (vs ADR 0003):** a postura conservador-pra-cima **restringe o caminho barato (`gpt-5.4-mini`) ao trivial mecânico (T1)**. A 0003 dizia "leve → modelo barato" de forma ampla; aqui só T1 cai no modelo leve, o resto é `gpt-5.4`.

## Exceção: crítica SEMPRE no modelo máximo (diretiva da Autoridade)

Quando o Executor é acionado no **papel de crítica**, o **modelo é sempre o maior aprovado pela Autoridade** (`gpt-5.5`), independente do tier/postura. Motivo: a crítica é o **gate de qualidade** (pega falsos-verdes e falhas de governança); ali queremos o máximo poder de modelo. Sobrepõe a regra geral "execução = modelos de execução".
...
| Conta como crítica → `gpt-5.5` | NÃO conta (segue a matriz de execução) |
|---|---|
...
| Ordem **híbrida** "critique e implemente" → **separar em 2 chamadas**: crítica em `gpt-5.5`, execução em modelo de execução conforme tier | |

- **Effort** segue o tier da crítica (governança/sensível = T4 → `high`; `xhigh` por gatilho). A diretiva fixa o **modelo**, não o effort.
- **Fallback se `gpt-5.5` indisponível:** NÃO cair silenciosamente para um modelo de execução (violaria a regra) → **escalar à Autoridade** (waiver explícito ou aguardar).
- **Guardrail:** qualquer lote com `gpt-5.5` concorrente acima do cap, ou qualquer `gpt-5.5/xhigh`, **avisa a Autoridade** antes (custo).
- **Calibração:** após 10–20 críticas em `gpt-5.5`, medir custo/latência/taxa de achados acionáveis/falso-positivo/consenso vs `gpt-5.4/high`, e reavaliar.
...
- **Guardrail de custo/paralelização:** lote com **> 6 chamadas concorrentes em `gpt-5.4/high`+** (ou qualquer `xhigh`) → **avisar a Autoridade antes** (apetite de custo é decisão dela). Cap default tunável.
- **Autoridade sobrepõe** qualquer escolha.

> **Template:** calibre o cap de concorrência (aqui, 6 chamadas em `gpt-5.4/high`+) ao apetite de custo e aos limites de rate/quota do seu projeto.
...
> **Atualização 2026-06-30:** esta política deixou de ser apenas referência dormente nesta instância. A ADR [[memory/decisions/0016-runtime-model-router-usage-ledger]] ativou o runtime `role+tier` via `.engrama/engine/scripts/model-router.sh`, `.engrama/engine/config/models.conf` e adapters em `.engrama/engine/adapters/`. Os ids de modelo continuam sendo configuração local, não verdade universal.
