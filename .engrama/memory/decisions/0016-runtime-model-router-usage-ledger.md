...
6. **Usage ledger local.** Cada execução pelo bridge que produz um stream JSONL de eventos escreve uma linha em `.engrama/evidence/usage/usage-YYYY-MM.jsonl` com schema `engrama.usage.v1`: projeto, branch, role, tier, adapter, provider, `model` efetivo, `configured_model`, `observed_model` quando o stream reportar, effort, billing mode, plano, timestamps UTC, duração, tokens quando disponíveis, turns, transcript e sessão. Plano/custo usam o modelo observado quando existir; se o stream não informar modelo, caem para o modelo configurado. Falhas iniciais do adapter sem eventos abortam antes de parsing/transcript de resposta/ledger, para não criar evidência sintética.
7. **Relatório local.** `.engrama/engine/scripts/usage-report.sh` sumariza o ledger por mês e por `model|role|tier|adapter`. `--month current` usa mês UTC, a mesma convenção usada para criar `usage-YYYY-MM.jsonl`. Tokens/preços ausentes aparecem como `unknown`, sem falhar.
8. **Billing por assinatura e API separados.** `.engrama/engine/config/subscriptions.conf` modela planos como Codex Pro; `.engrama/engine/config/prices.conf` guarda estimativas editáveis de API. Preços vazios não bloqueiam. No template distribuível, assinaturas nascem desativadas e sem valor mensal presumido; cada adotante precisa declarar a própria assinatura.
