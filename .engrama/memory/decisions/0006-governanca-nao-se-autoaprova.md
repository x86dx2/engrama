...
- **Bootstrap (chicken-and-egg, honesto):** o engrama **inicial** teve a crítica externa **dispensada** pela Autoridade (registrado como `dispensada` em [[evidence/qa/criticas-do-executor]]) — o gate precisava existir antes de poder exigir a prova de crítica. A partir do primeiro engrama, a regra passa a se aplicar a si mesma (toda edição de governança seguinte vai à crítica do Executor).
- **Validação cruzada estrutural:** a separação escritor≠auditor não é metáfora; é um fato de processo. O contraste útil é um caso como mem0, em que o mesmo modelo pode extrair e validar fatos. Aqui a tríade existe justamente para evitar essa autocorrelação. Ver [[memory/domain/validacao-cruzada-estrutural]].
...
1. Antes de comitar mudança que toque essas superfícies, a **crítica do Executor (gpt-5.5, read-only)** é obrigatória e registrada em [[evidence/qa/criticas-do-executor]] (por categoria, citando a branch).
...
> **Fronteira honesta do enforcement.** O hook local é **freio cooperativo**, não barreira inviolável: `git commit --no-verify`, `git -c core.hooksPath=/dev/null` e commits fora do harness do Orquestrador passam por cima dele; o `PreToolUse` só cobre o harness Claude. **Localmente, o modo estrito do diff-binding (`ENGRAMA_REQUIRE_DIFF_BIND=1`) fica OFF por padrão**; ligá-lo fora da CI continua sendo cooperação, não enforcement vinculante. A garantia vinculante exige **enforcement server-side**. A CI (`.github/workflows/ci.yml`) **reexecuta o gate contra o diff do PR** via `.engrama/engine/scripts/critique-gate-ci.sh` (reusa a mesma `classify()` + parsing do ledger por campo num repo sintético, mas injeta o fingerprint do **diff real do PR** via `engrama-diff-hash.sh --range <base>...HEAD`). O job `test` da CI (que **embute** o gate-contra-PR) está entre os ***required checks*** do *branch protection* (config de repositório/admin) — então o gate é **vinculante no merge**. O **modo estrito do diff-binding** fica **ligado na CI** (`ENGRAMA_REQUIRE_DIFF_BIND=1`). O gate garante que a crítica esteja **registrada**, não que um modelo independente de fato a tenha produzido — a independência é estrutural (Executor é processo separado), **não** provada pelo hook. **Furo R1:** localmente o mesmo autor pode escrever `confirmo` e comitar; fica **mitigado server-side** (required check ativo). *Ressalva:* em PRs com múltiplos commits, o binding cobre o **diff cumulativo** de `base...HEAD`, não cada commit isoladamente. Ver [[memory/decisions/0011-diff-binding-atestacao-verificavel]] e [[memory/gaps/auditoria-e-plano-de-remediacao]].
