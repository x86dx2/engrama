...
type: governance
status: active
touches: [memory/governance/index, memory/governance/continuidade-de-sessao, memory/governance/modelo-operacional]
date: 2026-06-20
source_refs:
  - CLAUDE.md
...
- **Dev:** N/A
- **Build:** shellcheck bin/*.sh .engrama/engine/scripts/*.sh
- **Test:** bash ./tests/run.sh
- **E2E:** N/A
- **Outros comandos operacionais:** `bash ./.engrama/engine/scripts/lint.sh`; `bash ./bin/bootstrap.sh /private/tmp/engrama-bootstrap-smoke`; `bash ./bin/sync-template.sh`; `git config core.hooksPath .engrama/engine/githooks`; `rg '\{\{[A-Z_]+\}\}'` para checar placeholders no artefato instalado.
...
- **Fluxo crítico do domínio:** instalação bootstrapavel limpa: `bin/bootstrap.sh` deve criar/inicializar repo-alvo, instalar `CLAUDE.md`, `AGENTS.md`, `.engrama/`, `.claude/settings.json`, substituir placeholders, ativar hooks e deixar `memory/project/bootstrap-do-projeto.md` pronto para entrevista de primeira abertura.
...
- `.engrama/engine/scripts/critique-gate.sh` adaptado para governança/gate/contract do Engrama.
