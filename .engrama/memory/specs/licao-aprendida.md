...
2. **Triar o destino durável** — escolher pelo menos um, do mais forte ao mais fraco:
   - **(a) Regra de gate** — novo caso em `classify()` ou no parser de `.engrama/engine/scripts/critique-gate.sh` **+ teste** (preferido para superfície sensível).
   - **(b) Regra de lint** — nova checagem em `lint.sh` **+ teste** (preferido para integridade de doc/estrutura).
   - **(c) Teste** — caso de regressão em `tests/` que fixa o comportamento correto (RED→GREEN; ver [[memory/specs/test-writing]]).
