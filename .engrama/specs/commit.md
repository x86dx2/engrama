...
source_refs:
  ...
  - /Users/x86/git-projects/engrama/.engrama/governance/modelo-operacional.md
---
...
## Antes de commitar
- [ ] **Auditei** o que está sendo commitado (não comito trabalho não auditado). Código → re-executei os gates (ADR [[decisions/0005-orquestrador-qa-reexecucao-e-metricas]]).
- [ ] **Engrama atualizado ANTES** do commit não-trivial: `log.md` (entry `## [data] tipo | título`) + página/ADR/gap conforme o que mudou.
- [ ] Se é **governança** → passou pela **crítica do Executor** (ADR [[decisions/0006-governanca-nao-se-autoaprova]]) com consenso (ou impasse escalado / dispensa da Autoridade).
...
- [ ] **Produção** (quando houver deploy): o Orquestrador **nunca aprova MR de prod**; escrita em prod = ordem + 2ª confirmação (ADR [[decisions/0009-producao-intocavel-dupla-confirmacao]]).
