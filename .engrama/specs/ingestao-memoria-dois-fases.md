...
type: spec
status: active
touches: [specs/README, decisions/0012-reconciliacao-de-memoria, governance/continuidade-de-sessao]
date: 2026-06-21
source_refs:
  - .engrama/CLAUDE.md
  - .engrama/decisions/0012-reconciliacao-de-memoria.md
...
- Nova página em `decisions/`, `governance/`, `specs/`, `domain/`, `gaps/` ou `roadmap/`.
...
1. **Escolher o tipo certo da página.**
   ...
   - `domain` para padrão/invariante nomeado desta instância.
...
3. **Garantir `source_refs` relativos à raiz.**
   - `domain/` sempre aponta para a fonte concreta no repo, nunca para uma lembrança vaga.
...
5. **Decidir se é página nova ou edição de página existente.**
   - Se a mudança só melhora um documento atual sem criar um novo nó de memória, editar o alvo pode ser mais correto que abrir outro arquivo.
...
Objetivo: comparar o candidato com o que **já** existe e declarar a operação correta em `reconcilia:`, conforme o ADR [[decisions/0012-reconciliacao-de-memoria]].
...
Se você estiver trabalhando no repo central do Engrama, pode repetir a busca em `template/.engrama` para auditar o framework distribuível também.
...
3. registrar a nova página em [[index]] e no índice de [[specs/README]], quando aplicável;
...
Este spec detalha o `Ingest` do `.engrama/CLAUDE.md`; o workflow curto continua lá, e a disciplina de memória quente/fria fica em [[governance/continuidade-de-sessao]].
