...
type: gap
status: proposed
touches: [decisions/0005-orquestrador-qa-reexecucao-e-metricas, decisions/0012-reconciliacao-de-memoria]
date: 2026-06-21
source_refs:
...
- **Staleness:** páginas `active` em `governance/`, `specs/` e `decisions/` podem emitir warning quando a última mudança versionada passar de 90 dias. A fonte é o último commit do arquivo, não o `mtime`.
...
- **Coerência semântica:** detectar duplicata, sobreposição ou contradição de significado continua dependendo de leitura humana, cross-links e `grep`. O campo `reconcilia:` do ADR [[decisions/0012-reconciliacao-de-memoria]] melhora o rastro, mas não substitui julgamento.
