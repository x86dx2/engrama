...
> **Fronteira honesta do enforcement.** O hook local é **freio cooperativo**, não barreira inviolável: `git commit --no-verify`, `git -c core.hooksPath=/dev/null` e commits fora do harness do Orquestrador passam por cima dele; o `PreToolUse` só cobre o harness Claude. A garantia vinculante exige **enforcement server-side**. A CI (`.github/workflows/ci.yml`) **reexecuta o gate contra o diff do PR** via `bin/critique-gate-ci.sh` (reusa a mesma `classify()` + parsing do ledger por campo, num repo sintético na branch do PR — driblando o detached-HEAD). O job `test` da CI (que **embute** o gate-contra-PR) está entre os ***required checks*** do *branch protection* (config de repositório/admin) — então o gate é **vinculante no merge**. O gate garante que a crítica esteja **registrada**, não que um modelo independente de fato a tenha produzido — a independência é estrutural (Executor é processo separado), **não** provada pelo hook. **Furo R1:** localmente o mesmo autor pode escrever `confirmo` e comitar; fica **mitigado server-side** (required check ativo). *Ressalva:* o **modo estrito do diff-binding** está **desligado** por um bug de consistência de fingerprint (ver [[decisions/0011-diff-binding-atestacao-verificavel]]) — o gate-contra-PR exige a crítica registrada, não o `sha256` exato. Ver [[gaps/auditoria-e-plano-de-remediacao]].
