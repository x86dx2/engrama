...
# .engrama/qa/criticas-do-executor.md continua manual e exige julgamento do
...
print_bootstrap_status() {
  ...
  case "$status_line" in
    *proposed*)
      printf 'BOOTSTRAP PENDENTE: .engrama/project/bootstrap-do-projeto.md esta com status proposed.\n'
      return 0
      ;;
    *)
      ;;
  ...
  if grep -q 'TODO' "$bootstrap_file" 2>/dev/null; then
    printf 'BOOTSTRAP PENDENTE: .engrama/project/bootstrap-do-projeto.md ainda tem TODO.\n'
    return 0
  fi
...
main() {
  ...
  bootstrap_file="$root/.engrama/project/bootstrap-do-projeto.md"
