...
- **Re-critica (codex-session `019f0a3e`): `confirmo`** — sem achado material novo; objecao #1 resolvida e DURAVEL (recompos o gate gerado, SHA bateu); schema/lint/diagrama cobertos com paridade byte-a-byte no template. **Consenso.**
- **Release-gate (ADR 0014):** superficie distribuivel mudou -> **waiver `sem-release`** (decisao da Autoridade, bound `sha256:ececf77f`) em [[evidence/qa/release-waivers]]. Committado (PR #19); CI 4 checks verdes.
- **Autoridade APROVOU o merge.** Flip `proposed -> active` da pagina dobrado nesta PR (evita PR de flip separado como no ADR 0015) -> rebind do critique-gate. PROXIMO: merge na main + delete branch; sync da main local.
