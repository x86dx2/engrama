...
if grep -Fq '.engrama/engine/scripts/critique-gate-ci.sh' "$TEMPLATE_CI"; then
  _r=0
else
  _r=1
fi
check S6B CORRETO "$_r" "ci do template referencia .engrama/engine/scripts/critique-gate-ci.sh"
