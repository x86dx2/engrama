...
> Template: substitua cada `> Template: comando do seu stack` pelo comando real (runner de teste, migrations/seed, dev server, deploy). Use `{{DEV_URL}}` como placeholder de host:porta local e ajuste-o às portas reais que você escolher. Mantenha a **separação de ambientes locais isolados** e o **gate de remoto/produção** — essas duas invariantes são universais, não as ferramentas.
...
|---|---|---|---|
| development | `> Template: comando do seu stack` | `{{DEV_URL}}` | banco/storage de development |
| staging-local | `> Template: comando do seu stack` | `{{DEV_URL}}` | banco/storage de staging |
| production-local | `> Template: comando do seu stack` | `{{DEV_URL}}` | banco/storage de production |
...
- **NÃO usar a porta de development** (pode ser sessão ativa da Autoridade). Caracterizar contra uma **porta dedicada** (`{{DEV_URL}}`).
...
- **Health:** `> Template: comando de health do seu stack` (ex.: `curl -s {{DEV_URL}}/health` → `{status:"healthy"...}`).
- **Matar:** `> Template: comando de teardown do seu stack` (ex.: matar pela porta/`{{DEV_URL}}` ou pelo config — ver a seção de teardown abaixo).
- **Golden:** `> Template: comando que roda o golden/contract apontando para {{DEV_URL}}`.
...
>
> Exemplo (troque pelo do seu projeto): um teardown que casava pelo nome de um script de dev compartilhado entre dois projetos derrubou junto a instância golden do projeto vizinho — o teardown precisa casar pelo **arquivo de config** ou pelo **path do projeto** ({{REPO_PATH}}), não pelo nome de script compartilhado. O incidente mostrou que `matar-pela-porta` sozinho **não basta**: sobram processos órfãos que se acumulam.
...
> Template: defina, para o seu projeto, o que conta como "read-only remoto" (livre) vs. "mutação remota" (gated) na sua `{{STACK}}` — comandos de deploy, flags `--remote` de escrita, rotação de secrets, migrations/seeds. Mantenha a invariante do ADR 0009.
...
- Sempre invocar o Executor com stdin fechado para evitar travas de boot: `{{EXECUTOR_CMD}} ... < /dev/null` (ver [[specs/executor-order]]).
