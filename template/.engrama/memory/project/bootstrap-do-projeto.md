...
- Adaptar `.engrama/engine/scripts/critique-gate.sh` (`classify()`) às superfícies reais do projeto.
- Confirmar/mesclar `.claude/settings.json` se o projeto já tiver config própria do Claude Code.
- Criar as primeiras páginas de `memory/domain/`, `memory/gaps/` e `memory/roadmap/` se o escopo do produto já estiver claro.
