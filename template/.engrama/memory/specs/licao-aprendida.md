...
type: spec
status: active
touches: [memory/governance/modelo-operacional, memory/decisions/0006-governanca-nao-se-autoaprova, evidence/qa/criticas-do-executor, memory/specs/README]
date: {{DATA}}
source_refs:
...
Playbook do **loop falha→regra**: como toda falha relevante vira uma **regra durável** (gate, lint, teste, ADR ou princípio), para que a memória institucional **componha** em vez de só acumular. Operacionaliza o princípio 12 de [[memory/governance/modelo-operacional]] (honestidade de claims).
...
- **Mesmo erro 2×** (sinal forte de regra faltando — espelha o anti-loop do [[memory/decisions/0004-executor-critica-ativa-discordancia-escala-a-autoridade]]).
...
2. **Triar o destino durável** — escolher pelo menos um, do mais forte ao mais fraco:
   - **(a) Regra de gate** — novo caso em `classify()`/parser de `.engrama/engine/scripts/critique-gate.sh` **+ teste** (superfície sensível).
   - **(b) Regra de lint** — nova checagem no linter **+ teste** (integridade de doc/estrutura).
   - **(c) Teste** — caso de regressão que fixa o comportamento correto (RED→GREEN; ver [[memory/specs/test-writing]]).
   - **(d) ADR** — quando a lição muda uma decisão/política (`memory/decisions/NNNN`).
   - **(e) Princípio/spec** — quando é norma transversal (`memory/governance/` ou um spec).
3. **Fechar** a lição: o commit que adiciona a regra **referencia o fato de origem** (na mensagem e/ou no `log.md`). Quando houve crítica, o ledger já registra o fato — mas **o destino da lição vive no commit/log, não num campo do ledger** (o ledger tem gramática fixa de 4 campos; ver [[evidence/qa/criticas-do-executor]]).
