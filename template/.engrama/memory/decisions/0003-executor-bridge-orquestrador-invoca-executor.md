...
## Decisão
- O Orquestrador monta a **ordem mínima** (ver [[memory/governance/continuidade-de-sessao]]) e chama `{{EXECUTOR_CMD}} --model <tier> [--output-schema]`.
...
**mecanizado**:
- `.engrama/engine/scripts/exec-bridge.sh` invoca `{{EXECUTOR_CMD}} --json`, salva a **ordem
  verbatim** em `.engrama/evidence/transcripts/<data>-<label>-order.md` e a **resposta íntegra**
  em `.engrama/evidence/transcripts/<data>-<label>-response.md`.
