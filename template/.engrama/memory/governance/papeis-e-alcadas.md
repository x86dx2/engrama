...
O Orquestrador **invoca o Executor diretamente** via {{EXECUTOR_CMD}} (sem relay humano de rotina), passando a **ordem mínima** (ver [[memory/governance/continuidade-de-sessao]]). O **roteamento pesado/leve** define o **modelo do Executor** (leve → {{MODELO_EXECUTOR_LEVE}}; pesado → {{MODELO_EXECUTOR_PESADO}}; trate os ids como exemplo/configuracao concreta e confirme o namespace real do adaptador), nunca se o Executor participa. O Executor devolve a **resposta crítica** (leitura/crítica/ajustes/execução/evidências/pendências). O Orquestrador **sempre audita** antes de comitar. Detalhe em [[memory/decisions/0003-executor-bridge-orquestrador-invoca-executor]].
