...
Cada invocacao via `.engrama/engine/scripts/exec-bridge.sh` gera um par datado em `.engrama/evidence/transcripts/`:

```
.engrama/evidence/transcripts/<YYYY-MM-DD>-<label>-order.md      # a ordem (verbatim)
.engrama/evidence/transcripts/<YYYY-MM-DD>-<label>-response.md   # a resposta do codex (integra) + cabecalho YAML
```

O cabecalho da resposta registra `codex-session`, modelo, sandbox e `label`.
A entrada correspondente no [ledger](../qa/criticas-do-executor.md)
