...
Uso:
  ...
  bash ./.engrama/scripts/engrama-diff-hash.sh --range <gitrange>

Imprime um fingerprint estavel do diff alvo, excluindo o ledger
.engrama/qa/criticas-do-executor.md. Sem flags (ou com --cached),
...
diff_stream() {
  case "$MODE" in
    cached)
      git diff --cached --raw -z -- . ':(exclude).engrama/qa/criticas-do-executor.md'
      ;;
    range)
      git diff --raw -z "$GIT_RANGE" -- . ':(exclude).engrama/qa/criticas-do-executor.md'
      ;;
    *)
