...
usage() {
  cat <<'EOF'
Uso: bash ./.engrama/engine/scripts/lint.sh [--report]
...
list_markdown_files() {
  find .engrama \
    -path '.engrama/evidence/transcripts' -prune -o \
...
check_wikilinks() {
  ...
  case "$file" in
    .engrama/log.md|.engrama/evidence/qa/criticas-do-executor.md)
      return 0
      ;;
    *)
      ;;
...
check_superseded_pointer() {
  ...
  while IFS='	' read -r line_no slug; do
    ...
    target="$(resolve_wikilink_target "$slug")" || continue
    case "$target" in
      "$ROOT/.engrama/memory/decisions/"*.md)
...
check_normative_markers() {
  ...
  case "$file" in
    .engrama/memory/governance/*.md|.engrama/memory/decisions/*.md) ;;
    *) return 0 ;;
  esac
...
check_staleness() {
  ...
  case "$file" in
    .engrama/memory/governance/*.md|.engrama/memory/specs/*.md|.engrama/memory/decisions/*.md) ;;
    *) return 0 ;;
  esac
...
is_listed_in_primary_indexes() {
  ...
  for index_file in .engrama/index.md .engrama/memory/governance/index.md; do
...
check_orphan_pages() {
  ...
  while IFS= read -r file; do
    ...
    report_problem "$file" 1 "pagina orfa: sem wikilink em outro .md e fora de .engrama/index.md/.engrama/memory/governance/index.md"
...
check_adr_numbering_gaps() {
  ...
  for file in .engrama/memory/decisions/*.md; do
    [ -f "$file" ] || continue
    base="$(basename "$file")"
    case "$base" in
      [0-9][0-9][0-9][0-9]-*.md)
  ...
  report_problem ".engrama/memory/decisions" 1 "gap na numeracao de ADRs: faltando $missing_numbers"
}
